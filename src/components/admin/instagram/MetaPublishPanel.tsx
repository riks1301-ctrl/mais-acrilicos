...
export function MetaPublishPanel({ postId, status, format, publicationChannel, instagramMediaId, metaPublishError, onPublished }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/instagram/posts/${postId}/publish-meta`).then((r) => r.json()).then(setEligibility).catch(() => {});
  }, [postId]);

  async function publishNow() {
    if (!confirm("Publicar AGORA via Meta Graph API? Esta ação publica no Instagram real (modo ATIVO).")) return;
    setLoading(true);
    setMessage(null);
    ...
    else setMessage(`Publicado! Media ID: ${data.mediaId}`);
    onPublished();
  }
  ...
  return (
    <section className="rounded-2xl border border-blue-200 bg-blue-50/50 p-6 space-y-4">
      ...
      <p className="text-sm text-slate-600">
        API oficial — sem Selenium. Requer aprovação, imagem HTTPS pública e modo Meta configurado.
      </p>
      ...
      {meta && (
        <div className="flex flex-wrap gap-2 text-xs">
          ...
          <Badge>Auto: {meta.envAutoPublish && meta.autoPublish ? "ON" : "OFF"}</Badge>
        </div>
      )}

      {instagramMediaId && <p className="text-sm text-green-700">Último media ID: {instagramMediaId}</p>}
      ...
      {unsupported && (
        <p className="rounded-xl bg-amber-100 px-3 py-2 text-sm text-amber-900">
          UNSUPPORTED_BY_CURRENT_IMPLEMENTATION — Stories/Reels não são publicados automaticamente nesta versão.
        </p>
      )}

      {carouselPrep && (
        <p className="rounded-xl bg-amber-100 px-3 py-2 text-sm text-amber-900">
          Carrossel via Meta API em fase preparada — use Feed com imagem única ou publicação manual no app.
        </p>
      )}
      ...
      {eligibility?.imageUrl && <p className="text-xs text-slate-500 break-all">Imagem: {eligibility.imageUrl}</p>}

      <button
        type="button"
        onClick={publishNow}
        disabled={loading || !["APPROVED", "SCHEDULED"].includes(status) || unsupported || carouselPrep}
        className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
      >
        {loading ? "Publicando..." : "Publicar agora via Meta API"}
      </button>
